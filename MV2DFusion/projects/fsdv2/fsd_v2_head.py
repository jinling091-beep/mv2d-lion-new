...
class FSDV2Head(SparseClusterHead):
    ...
    def modify_gt_for_single_task_single_sample(self, gt_bboxes_3d, gt_labels_3d, task_id):
        ...
        for i, name in enumerate(class_names_this_task):
            ...
            out_gt_bboxes_list.append(gt_bboxes_3d[this_cls_mask.to(gt_bboxes_3d.device)])
    ...
    def get_targets_single(self,
        ...
        if num_cluster == 0:
            iou_labels = None
            if self.loss_iou is not None:
                iou_labels = voxel_xyz.new_zeros(0)
        ...
        gt_bboxes_3d = gt_bboxes_3d[valid_gt_mask.to(gt_bboxes_3d.device)]
        # gt_labels_3d = gt_labels_3d[valid_gt_mask]
        gt_labels_3d = gt_labels_3d[valid_gt_mask.to(gt_labels_3d.device)]
    ...
    def get_bboxes(self,
        ...
        if all_task_max_num is not None:
            new_out_list = []
            for b_idx in range(batch_size):
                bboxes = concat_list[b_idx][0]
                scores = concat_list[b_idx][1]
                labels = concat_list[b_idx][2]
                feats = concat_list[b_idx][3]
                if len(scores) > all_task_max_num:
                    topk_inds = scores.topk(all_task_max_num)[1]
                    bboxes = bboxes[topk_inds]
                    scores = scores[topk_inds]
                    labels = labels[topk_inds]
                    feats = feats[topk_inds]
                new_out_list.append((bboxes, scores, labels, feats))
            concat_list = new_out_list
    ...
    def get_bboxes_single_task(
        ...
        voxel_xyz_list = self.split_by_batch(voxel_xyz, batch_inds, batch_size)
        voxel_feat_list = self.split_by_batch(voxel_feat, batch_inds, batch_size)
    ...
    def _get_bboxes_single(
        ...
        if len(cls_logits) == 0:
            ...
            return (out_bboxes, out_scores, out_labels, out_feats)
