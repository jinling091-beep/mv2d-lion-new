...
class CustomNuScenesDataset(NuScenesDataset):
    ...
    #                      metric='bbox',
    #                      result_name='pts_bbox'):
    #     """使用自定义评估脚本 eval_distance.py 进行评估。"""
        
    #     # 1. 导入你的自定义评估器类
    #     # 注意：请确保 eval_distance.py 在你的 Python 搜索路径中，或者在同一目录下

    #     # 2. 获取测试时使用的 .pkl 标注文件路径
    #     # 在 MMDetection3D 中，self.ann_file 存储了数据集标注文件的路径
    #     pkl_path = self.ann_file
        
    #     print(f"\n[Custom Eval] Starting evaluation using custom script...")
    #     print(f"[Custom Eval] Result Path: {result_path}")
    #     print(f"[Custom Eval] Annotation Path: {pkl_path}")

    #     if not os.path.exists(result_path):
    #         print(f"Error: Result file {result_path} not found.")
    #         return dict()

    #     # 使用 mmcv 的 get_root_logger 确保信息写入文件
    #     from mmcv.utils import get_logger
    #     if logger is None:
    #         logger = get_logger('mmdet3d')

    #     logger.info(f"\n[Custom Eval] Starting evaluation...")

    #     # 3. 实例化并运行评估逻辑
    #     # 默认 max_dist 设为 54.4，你可以根据自制数据集的需求调整
    #     evaluator = NuScenesStandaloneEvaluator(max_dist=54.4)
        
    #     # 该方法内部会打印表格并输出 mAP 和 NDS
    #     eval_results = evaluator.evaluate(result_path, pkl_path)

    #     # 4. 返回一个空字典或提取核心指标返回给训练框架
    #     # 由于你的 evaluator.evaluate 主要是打印输出，
    #     # 如果需要将指标记录到 Tensorboard 或日志，你需要让 evaluate 返回一个结果字典。
    #     ret_metrics = {}
    #     for k, v in eval_results.items():
    #         ret_metrics[f'{result_name}_{k}'] = v
        
    #     # 显式打印到日志文件
    #     logger.info(f"Custom Evaluation Results: {ret_metrics}")

    #     return ret_metrics
    #     # return {'pts_bbox_NuScenes/Custom_Eval_Done': 1.0}
