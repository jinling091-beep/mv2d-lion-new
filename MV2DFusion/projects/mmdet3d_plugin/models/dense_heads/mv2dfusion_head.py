...
class MV2DFusionHead(AnchorFreeHead):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        # ---- 新增：新加的解码层参数从最后一块预训练层复制，而非随机初始化 ----
        # 场景：6 层权重加载进 7 层模型时，第 7 层(index=num_pred-1)在
        # checkpoint 中缺失、默认保持随机初始化。这里改为把最后一块预训练层
        # (index=num_pred-2，即第 6 层)的参数复制到第 7 层，加快收敛。
        # resume 7 层权重时 dst 层参数已存在，会自动跳过。
        if strict or self.num_pred < 2:
            return
        src = self.num_pred - 2
        dst = self.num_pred - 1
        dst_markers = (
            f'transformer.decoder.layers.{dst}.',
            f'cls_branches.{dst}.',
            f'reg_branches.{dst}.',
            f'dyn_q_prob_branch.{dst}.',
        )
        if any(k.startswith(m) for m in dst_markers for k in state_dict):
            return

        self.transformer.decoder.layers[dst].load_state_dict(
            self.transformer.decoder.layers[src].state_dict())
        self.cls_branches[dst].load_state_dict(
            self.cls_branches[src].state_dict())
        self.reg_branches[dst].load_state_dict(
            self.reg_branches[src].state_dict())
        self.dyn_q_prob_branch[dst].load_state_dict(
            self.dyn_q_prob_branch[src].state_dict())

        # print_log(
        #     f'[MV2DFusionHead] checkpoint 缺少第 {dst} 层解码器参数，'
        #     f'已从第 {src} 层复制参数初始化（含 decoder/cls/reg/dyn_q_prob 分支）',
        #     logger='current')
    ...
    def forward(self, img_metas, dyn_query=None, dyn_feats=None,
        ...
        #decoder.layer6
        # if not getattr(self, '_dbg_grad1', False):
        #     self._dbg_grad1 = True
        #     print_log(f'[grad-dbg] after-transformer outs_dec.requires_grad='
        #               f'{outs_dec.requires_grad}', logger='current')
        outputs_classes = []
        outputs_coords = []
        ...
        # #decoder.layer6
        # if not getattr(self, '_dbg_grad2', False):
        #     self._dbg_grad2 = True
        #     print_log(f'[grad-dbg] after-mask cls={all_cls_scores.requires_grad} '
        #               f'bbox={all_bbox_preds.requires_grad}', logger='current')
