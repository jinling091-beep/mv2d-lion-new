...
class MV2DFusion(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
        ...
        if not requires_grad:
            ...
            pts_gt_bboxes_3d, pts_gt_labels_3d = [], []
            if getattr(self.pts_backbone, 'use_fsd_anno', True):
                for b in range(B):
                    box, label = self.convert_to_fsd_anno(gt_bboxes_3d[b], gt_labels_3d[b])
                    pts_gt_bboxes_3d.append(box)
                    pts_gt_labels_3d.append(label)
            else:
                pts_gt_bboxes_3d = gt_bboxes_3d
                pts_gt_labels_3d = gt_labels_3d
            out_dict = self.pts_backbone.forward_train(data['pts_feats'], img_metas, pts_gt_bboxes_3d, pts_gt_labels_3d)
